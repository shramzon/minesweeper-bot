...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        if text in ["помощь", "help", "команды", "чё делать"]:
            ...
            if len(parts) == 3:
                try:
                    ...
                    return
                if mines < 1 or mines > 24 or bet < 10:
                    await update.message.reply_text("❌ Мин: 1-24, ставка: ≥10")
                    return
                if user["balance"] < bet:
                    await update.message.reply_text(f"❌ Недостаточно мун! Баланс: {user['balance']}")
                    return
        ...
        # Ничего не делаем — молча игнорируем неизвестные команды
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        if data.startswith("bet_"):
            ...
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("SELECT board, revealed, mines, bet, opened, game_over FROM games WHERE gid=?", (gid,))
            row = c.fetchone()
            if not row:
                conn.close()
                return
            board_str, revealed_str, mines, bet, opened, game_over = row
            board = eval(board_str)
            revealed = eval(revealed_str)
            ...
            revealed[r][c] = True
            new_opened = sum(sum(row) for row in revealed)
            is_mine = board[r][c]

            c.execute("UPDATE games SET revealed=?, opened=? WHERE gid=?", (str(revealed), new_opened, gid))
            conn.commit()
            conn.close()

            if is_mine:
                c.execute("UPDATE games SET game_over=1 WHERE gid=?", (gid,))
                conn.commit()
                await query.edit_message_text(
                    f"💥 БОМБА! -{bet} мун",
                    reply_markup=InlineKeyboardMarkup([
            ...
            total_safe = 25 - mines
            if new_opened == total_safe:
                mult = calc_multiplier(new_opened, 25, mines)
                win = int(bet * mult)
                update_user(uid, balance=get_user(uid)["balance"] + win, games_won=get_user(uid)["games_won"] + 1, total_won=get_user(uid)["total_won"] + win)
                c.execute("UPDATE games SET game_over=1, won=1 WHERE gid=?", (gid,))
                conn.commit()
                await query.edit_message_text(
                    f"🎉 ВСЕ КЛЕТКИ! x{mult}\n+{win} мун",
                    reply_markup=InlineKeyboardMarkup([
            ...
            mult = calc_multiplier(new_opened, 25, mines)
            await query.edit_message_text(
                f"💰 x{mult}\n💎 {new_opened}/{total_safe}",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("❓", callback_data=f"cell_{gid}_{(r+1)%5}_{c}")]
                ])
            )

        elif data.startswith("buy_"):
            farm_type = data.split("_")[1]
            price = {"small": 2000, "medium": 5000, "large": 10000}[farm_type]
            user = get_user(uid)
            ...
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("INSERT OR REPLACE INTO farms (uid, type, last_collect) VALUES (?, ?, ?)", (uid, farm_type, time.time()))
            conn.commit()
            conn.close()
