...
✅ Убираю сообщение "Неизвестная команда"  
✅ Делаю иконки клеток как `❓`  
✅ Остальное — как есть (чистый стиль)

---

## 📦 Новый `bot.py` (идеальный + тихий + `❓`):
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        # --- НЕИЗВЕСТНАЯ КОМАНДА (тихо, без сообщения) ---
        # (ничего не делаем, просто молча игнорируем)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        if data.startswith("bet_"):
            ...
            update_game(gid, revealed=str(revealed), opened=opened)

            # Обновляем статус пользователя
            user = get_user(uid)
            if is_mine:
                update_game(gid, game_over=1)
                await query.edit_message_text(
                    f"💥 БОМБА! -{game[3]} мун",
                    reply_markup=InlineKeyboardMarkup([
            ...
            # Проверяем, все ли безопасные открыты
            total_safe = 25 - game[2]
            if opened == total_safe:
                mult = calc_multiplier(opened, 25, game[2])
                win = int(game[3] * mult)
                update_user(uid, balance=user["balance"] + win, games_won=user["games_won"] + 1, total_won=user["total_won"] + win)
                update_game(gid, game_over=1, won=1)
                await query.edit_message_text(
                    f"🎉 ВСЕ КЛЕТКИ! x{mult}\n+{win} мун",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("❓", callback_data="ignore")]
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("❓", callback_data=f"cell_{gid}_{(r+1)%5}_{c}"),
                     InlineKeyboardButton("💰 x{mult}", callback_data=f"cashout_{gid}")]  # Пример, можно улучшить
                ])
            )
            ...
            user = get_user(uid)
            update_user(uid, balance=user["balance"] + win, games_won=user["games_won"] + 1, total_won=user["total_won"] + win)
            update_game(gid, game_over=1, cashed_out=1)
            await query.edit_message_text(
                f"💰 ЗАБРАЛ x{mult}!\n+{win} мун",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("❓", callback_data="ignore")]
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TE